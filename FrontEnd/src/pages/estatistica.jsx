...
export default function Estatisticas() {
    ...
    const estatisticas = async () => {
        const response = await fetch("http://localhost:3333/estatistic");
        if (!response.ok) {
    ...
    return (
        // listar estatisticas, ao lado tera botao de excluir, editar
        <div className="flex flex-col items-center justify-center w-[80%] bg-blue-400 p-8 rounded-lg shadow-lg mt-10">
            <h1 className="text-3xl text-white font-bold mb-6">ESTATISTICAS</h1>
            <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
                ...
                <tbody>
                    ...
                    estatisticas.map((estatistica) => {
                        return(
                            <tr key={estatistica.id} className="border-b text-black border-gray-200 hover:bg-gray-100">
                                <td className="py-1 px-6 text-left">{estatistica.name}</td>
            ...
            <div className="mt-6">
                <a href="/estatisticas/criar" className="bg-white text-black px-4 py-2 rounded hover:bg-green-500 hover:text-white transition-colors duration-300 cursor-pointer">
                    Adicionar Jogador
                </a>
            </div>
        </div>
